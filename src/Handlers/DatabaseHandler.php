...
class DatabaseHandler extends AbstractSessionHandler {
	...
	public static function createTable() {
		...
		$created_db_version = get_option( 'wenprise_session_db_version', '0.0' );
		...
		$created = maybe_create_table(
			"{$wpdb->prefix}rs_sessions",
			"CREATE TABLE {$wpdb->prefix}rs_sessions (
	...
	protected function directWrite( string $key, string $data, int $expires = null ) {
		...
		return $wpdb->replace( "{$wpdb->prefix}rs_sessions", $session );
	}
	...
	protected function directRead( string $key ) {
		...
		$session = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}rs_sessions WHERE session_key = %s",
				$session_key
			),
			ARRAY_A
		);
	...
	protected function directDelete( string $key ) {
		...
		if ( null !== $wpdb ) {
			$wpdb->delete( "{$wpdb->prefix}rs_sessions", [ 'session_key' => $session_key ] );
		}
	}
	...
	public static function directClean() {
		...
		if ( null !== $wpdb ) {
			$wpdb->query(
				$wpdb->prepare(
					"DELETE FROM {$wpdb->prefix}rs_sessions WHERE session_expiry < %s LIMIT %d",
					time(),
					1000
				)
			);
